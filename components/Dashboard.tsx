...
//           <button className=" bg-black-200 p-2 rounded-md z-30 shadow-sm shadow-white-100 text-purple ">
//             Finalize 
//           </button>
//         </div>
...
import { RootState } from "@/redux/store";
import { useFormContext } from "./FormContext";
import { clearCalculations, addCalculation } from '@/redux/slices/calculationSlice';
import { clearUser } from '@/redux/slices/authSlice';

const Dashboard: React.FC = () => {
  const [showDetails, setShowDetails] = useState<number | null>(null);
  const { calculations } = useSelector((state: RootState) => state.calculation);
  const user = useSelector((state: RootState) => state.auth.user);
  ...
  const handleDelete = (index: number) => {
    ...
    updatedCalculations.forEach((calculation) => {
  ...
  return (
    <div className="pt-28">
          ...
          <table className="w-full border border-gray-500">
            <thead className="bg-black-200 border-b-[1px] border-gray-500">
              <tr>
                ...
                <th className="p-3 text-sm font-semibold tracking-wide text-left">Grade Point</th>
                <th className="p-3 text-sm font-semibold tracking-wide text-left">Year</th>
                <th className="p-3 text-sm font-semibold tracking-wide text-left">View</th>
            ...
            <tbody>
              {calculations.map((calculation, index) => (
                <tr key={index} className="border-b-[1px] border-gray-500">
                  <td className="p-3 text-sm text-white-100">{calculation.level} Level</td>
                  <td className="p-3 text-sm text-white-100">{calculation.semester} Semester</td>
                  <td className="p-3 text-sm text-white-100">{calculation.gradePoint} (GP)</td>
                  <td className="p-3 text-sm text-white-100">{calculation.year}</td>
                  <td
                  ...
                  <td
                    ...
                    className="p-3 text-sm text-red-600 cursor-pointer"
                  >
                    Delete
                  </td>
                </tr>
      ...
      {showDetails !== null && calculations[showDetails] && (
        <div className="mt-8">
          <h2 className="text-lg font-semibold mb-4">
            Calculation Details for {calculations[showDetails].level} Level, {calculations[showDetails].semester} Semester
          </h2>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-black-200 border border-gray-500">
              <thead>
                <tr>
                  <th className="px-4 py-2 border-b border-gray-500 text-left">Course Code</th>
                  <th className="px-4 py-2 border-b border-gray-500 text-left">Course Unit</th>
                  <th className="px-4 py-2 border-b border-gray-500 text-left">Course Grade</th>
                </tr>
              </thead>
              <tbody>
                {calculations[showDetails].calculationDetails.map((detail, idx) => (
                  <tr key={idx}>
                    <td className="px-4 py-2 border-b border-gray-500">{detail.code}</td>
                    <td className="px-4 py-2 border-b border-gray-500">{detail.unit}</td>
                    <td className="px-4 py-2 border-b border-gray-500">{detail.grade}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="flex justify-center mt-12">
        <Link href="/result">
          <MagicButton title="Calculate New Grade Point" />
        </Link>
      </div>
    </div>
  );
